...
func writeError(w http.ResponseWriter, httpCode int, err error) error {
	...
	if errWithCode, ok := err.(model.ErrorWithCode); ok {
...
func FromHTTPCode(w http.ResponseWriter, httpCode int) error {
	...
	response := CommonResponse{
		Data:  nil,
		Error: &model.Error{ErrCode: statusCode, Message: statusText},
	}
...
func FromPaginate[T any](w http.ResponseWriter, paginate model.PaginateResult[T]) error {
	data := paginate.Data
	if data == nil {
